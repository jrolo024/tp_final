  ...
  React.useEffect(() => { //recup auto la geoloc et mettre à jour le nom de la ville direct
    ...
    const getLocation = () => {
      ...
      if (navigator.geolocation) {
        ...
        setError("erreur");
        setLoading(false);
        console.error("La géolocalisation n'est pas dispo");
      }
    };
  ...
  const getMeteo = async () => { //appel api meteo
    try {
      ...
      if (response.ok) {
        ...
        setLoading(false);
        setError("error");
        console.error(`Erreur ${response.status}: Impossible d'obtenir la météo.`);
      }
    } catch (error) {
      setLoading(false);
      setError("error");
  ...
  const getCity = async () => { //appel api ville
    try {
      ...
      if (response.ok) {
        ...
        console.error(`Erreur ${response.status}: Impossible d'obtenir la ville.`);
      }
    } catch (error) {
      console.error('Erreur lors de la recup de la ville :', error);
    }
  };
